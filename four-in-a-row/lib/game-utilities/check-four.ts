...
function followDirection(board, col, row, direction, playerId, pieces = [], reversed = false) {
  ...
  const notOnBoard = board[nextPieceCol] === undefined
                     || board[nextPieceCol][nextPieceRow] === undefined;

  // If hit edge, reverse
  // If on second run, don't reverse directions
  if (!reversed && (notOnBoard || (playerId !== board[nextPieceCol][nextPieceRow]))) {
  ...
  const nextPiecePlayerId = board[nextPieceCol][nextPieceRow];
